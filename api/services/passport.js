...
passport.endpoint = function(req, res) {
    ...
    if (!strategies.hasOwnProperty(provider)) {
        return res.status(401).send('unauthorized'); //redirect('/login');
    }
...
passport.update = function(req, res, newPass, next) {
    ...
    User.findOne({
            ...
            this.protocols.local.update(req, res, user, newPass, function(err, newpass){
                next(err, newPass, user);
            });
        }
    );
};
...
passport.callback = function(req, res, next) {
    ...
    if (provider === 'local' && action !== undefined) {
        ...
        if (action === 'disconnect' && req.user) {
            ...
            // has failed.
            this.authenticate(provider, next)(req, res, next);
        }
    }
};
